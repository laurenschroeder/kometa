// Fire Emoji Comet — archived from comet-system.ts
// Smiley face sphere head + organic 3D fire tube body + spring physics
import {
  AdditiveBlending,
  BufferAttribute,
  BufferGeometry,
...
export class FireEmojiComet extends createSystem({}) {
  ...
  init() {
    ...
    this.world.createTransformEntity(lFire);
    this.world.createTransformEntity(rFire);
  }
  ...
  private _buildTube(trail: Float32Array, verts: Float32Array, colors: Float32Array,
    pa: BufferAttribute, ca: BufferAttribute, time: number): void {
    for (let i = 0; i < TUBE_SAMPLES; i++) {
      ...
      this._tangent.set(trail[pi]-trail[ni], trail[pi+1]-trail[ni+1], trail[pi+2]-trail[ni+2]);
      const tLen = this._tangent.length();
      if (tLen < 0.0001) this._tangent.set(0, 1, 0); else this._tangent.divideScalar(tLen);
      ...
      const r = b, g = b * Math.max(0, 0.72 - t * 0.75);
      for (let j = 0; j < TUBE_SIDES; j++) {
        ...
        const ca2 = Math.cos(angle), sa = Math.sin(angle);
        ...
        colors[vi] = r; colors[vi+1] = g; colors[vi+2] = 0;
      }
    }
    pa.needsUpdate = true;
    ca.needsUpdate = true;
  ...
  private _step(state: CometState, grip: Object3D, flexion: number, dt: number): void {
    ...
    if (flexion >= SNAP_THRESHOLD) {
      ...
      state.pos.copy(this._handPos); state.prevHand.copy(this._handPos); state.wasSnapped = true; return;
    }
